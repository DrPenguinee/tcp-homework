...
class TCP_packet:
    HEADER_SIZE = 13
    MSS = 10
    # maximum segment size; the max of the data
    # HEADER_FORMAT explanation:
    # seq_num == 4 bytes, ack_num == 4 bytes, header_len == 2 bytes, ACK == 1 byte, SYN == 1 byte, FIN == 1 byte,
    HEADER_FORMAT = '2IH3b' + str(MSS) + 's'
    ...
    def pack_segment(self) -> bytes:
        empty_size = self.MSS - len(self.data)
        self.data += b' ' * empty_size
    ...
    def unpack_segment(cls, packed_segment):
        (seq_num, ack_num, header_size, ACK, SYN, FIN, data) = struct.unpack(TCP_packet.HEADER_FORMAT, packed_segment)
        data = data.rstrip(b' ')
